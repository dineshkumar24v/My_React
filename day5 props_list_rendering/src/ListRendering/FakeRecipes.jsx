...
export function FakeRecipes(){
  ...
  return(
    ...
    {/* Here, data is an object, and the actual array you want to iterate over is data.recipes. */}
      {data.recipes.map((recip)=>(
        <div className="cards">
          <img src={recip.image} height={200}/>
          <h3>{recip.name}</h3>
          <p>{recip.cuisine}</p>
          <p><b>Ingredients:</b> {recip.ingredients}</p>
        </div>
      ))}
    </div>
  )
